...
3. **权限**：`check_permissions` 给三态：`allow`（放行）/ `deny`（拒绝）/ `ask`（弹窗问人）。SkillSearch 没写权限逻辑，`TOOL_DEFAULTS` 兜底 passthrough：工具自己不表态，放不放行由宿主的全局规则和权限模式决定，工具作者一行不用写。
...
- **入口层（第 4 章）**：交互入口。自动接入是最基础的一条路：登记进注册表，Agent 自动就位；其余的路按需挑选，REPL 的 `/skills`、headless、远程 API、orchestrator 自动化，都能接到同一台引擎上。**入口可以无数个，底下的引擎永远只有一个。**
